...
module Mangar
  ...
  mattr_accessor :collection, :dir, :mangar_dir, :books_dir, :videos_dir, :deleted_dir, :exported_dir
  ...
  def self.configure(collection)
    ...
    Mangar.exported_dir = File.expand_path("#{Mangar.mangar_dir}/exported")

    Dir.mkdir(Mangar.mangar_dir) if !File.exists?(Mangar.mangar_dir)
    [Mangar.books_dir, Mangar.videos_dir, Mangar.deleted_dir, Mangar.exported_dir].each { |d| FileUtils.mkdir_p(d) unless File.exists?(d) }
