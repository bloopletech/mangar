...
class Video < Item
  ...
  def self.import_and_update
    ...
    cmd = <<-CMD
cd #{File.escape_name(Mangar.dir)} && find . -depth -type f -o \\( -type f \\( #{File::VIDEO_EXTS.map { |ext| "-iname '*#{ext}'" }.join(' -o ')} \\) \\)
CMD
  ...
  def self.import(relative_path) 
    ...
    title = File.basename(relative_path).gsub(/_/, ' ')
  ...
  def self.preview_from_video_file(real_path)
    ...
    return File.open(frame_filename, "r")
  end
end